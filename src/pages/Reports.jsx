...
function exportReportCSV(data, period, businessName) {
    const rows = [['Report', `${businessName} — ${data.periodLabel || period}`], []]
    ...
    rows.push(['Transactions', data.salesCount])
    rows.push(['Cost of Goods', data.totalCost])
    rows.push(['Total Expenses', data.totalExpenses])
    rows.push([])
    if (data.topProducts?.length > 0) {
        rows.push(['TOP PRODUCTS', ''])
        rows.push(['Product', 'Units Sold', 'Revenue (UGX)', 'Profit (UGX)'])
    ...
    rows.push(['In Stock', data.stockSummary?.inStock])
    rows.push(['Low Stock', data.stockSummary?.lowStock])
    rows.push(['Out of Stock', data.stockSummary?.outOfStock])
    rows.push(['Stock Value (UGX)', data.stockSummary?.totalValue])
    ...
    a.href = url
    a.download = `${businessName || 'stocktracer'}-report-${new Date().toISOString().slice(0, 10)}.csv`
